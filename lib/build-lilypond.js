...
export const config = {
  ...
  masterFile: {
    title: 'Master file',
    description: 'Specify a master file from which your output may be compiled. This will generate a build target for the file.',
    type: 'string',
    default: 'master.ly',
    order: 1
  },
...
export function provideBuilder() {
  ...
  return class LilypondBuildProvider extends EventEmitter {
    constructor(cwd) {
      ...
      atom.config.observe('build-lilypond.masterFile', () => this.emit('refresh'));
    }
    ...
    isEligible() {
      const requiredFiles = [ 'definitions.ly' ]
    ...
    settings() {
      ...
      const masterFile = atom.config.get('build-lilypond.masterFile').trim();
      if (masterFile.length != 0) {
        targets.push(
          {
            name: `LilyAtom: default (${masterFile})`,
            exec: 'lilypond',
            args: customArguments.concat(masterFile),
            sh: true,
            cwd: this.cwd,
